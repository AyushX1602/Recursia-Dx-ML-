...
router.post('/demo-analysis', catchAsync(async (req, res) => {
  ...
  // ================================================================
  // REAL ML PREDICTION via GigaPath API
  // ================================================================
  let mlResult = null;

  try {
    console.log('🧠 Sending image to GigaPath ML service for REAL prediction...');
  ...
  if (!mlResult) {
    return res.status(503).json({
      success: false,
      error: 'ML service unavailable. Please ensure GigaPath API is running on port 5002.',
      message: 'Start GigaPath API with: cd ml/api && python gigapath_api.py --port 5002'
    });
  }
