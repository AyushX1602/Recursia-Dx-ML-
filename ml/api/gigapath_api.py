...
def validate_checkpoint(checkpoint_path):
    """
    Validate that checkpoint file exists and can be loaded.
    ...
    if not checkpoint_path.exists():
        ...
        logger.error("=" * 60)
        logger.error(f"The specified checkpoint does not exist:")
        logger.error(f"  {checkpoint_path.resolve()}")
        logger.error("")
        logger.error("Please ensure the checkpoint file exists or use auto-discovery")
        logger.error("by running from the project root without --checkpoint_path")
        logger.error("=" * 60)
        return False
...
EXAMPLE:
  python gigapath_api.py --checkpoint_path checkpoints/best_model.pth --port 5002
    ...
    if not args.checkpoint_path:
        ...
        script_dir = Path(__file__).resolve().parent  # ml/api/
        ml_dir = script_dir.parent  # ml/
        project_root = ml_dir.parent  # RecursiaDx/ (project root)
        
        # Also try current working directory as project root
        cwd_root = Path.cwd()
        if cwd_root.name == 'ml':
            cwd_root = cwd_root.parent
        
        default_paths = [
            # Centralized weights folder (PREFERRED - portable)
            ml_dir / 'models' / 'weights' / 'tumor_gigapath_mil.pth',
            # From project root (fallback for backward compatibility)
            project_root / 'GigaPath-AI-WSI-Breast-Cancer-Lesion-Analysis' / 'checkpoints' / 'best_model.pth',
            # From cwd (if running from different location)
            cwd_root / 'GigaPath-AI-WSI-Breast-Cancer-Lesion-Analysis' / 'checkpoints' / 'best_model.pth',
            # Relative to ml folder
            ml_dir / '..' / 'GigaPath-AI-WSI-Breast-Cancer-Lesion-Analysis' / 'checkpoints' / 'best_model.pth',
            # Local checkpoints folder
            script_dir / 'checkpoints' / 'best_model.pth',
            Path('checkpoints') / 'best_model.pth',
            # Parent directories
            Path('..') / 'GigaPath-AI-WSI-Breast-Cancer-Lesion-Analysis' / 'checkpoints' / 'best_model.pth',
            Path('..') / '..' / 'GigaPath-AI-WSI-Breast-Cancer-Lesion-Analysis' / 'checkpoints' / 'best_model.pth',
        ]
        
        logger.info("Searching for checkpoint in default locations...")
        for path in default_paths:
            resolved_path = path.resolve()
            if resolved_path.exists():
                args.checkpoint_path = str(resolved_path)
                logger.info(f"Found checkpoint at: {resolved_path}")
                break
        
        if not args.checkpoint_path:
            logger.error("Could not find checkpoint in any default location:")
            for path in default_paths:
